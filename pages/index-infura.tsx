...
const getAddresses = (contractAddress: string) => {
    // prettier-ignore
    ...
    let filter = {
        address: contractAddress,
        fromBlock: 7102532, // Block of Contract's first transaction
        toBlock: 'latest',
        topics: []
    }
...
const Home: NextPage<{ saintAddresses: string[] }> = ({ saintAddresses }) => {
    const listItems = saintAddresses.map(address => (
...
Home.getInitialProps = async ({ req }) => {
    const saintAddresses = await getAddresses(SAINT_TOKEN)
    // const boiAddresses = await getAddresses(BOI_TOKEN)

    // console.log(`*** BOI ADDRESSES: \n${boiAddresses} ****`)

    // const finances = await getFinances()

    return { saintAddresses }
}

export default Home

// provider.resetEventsBlock(8960961)
// let contract = new ethers.Contract(BOI_TOKEN, abi, provider)

// // A filter from me to anyone
// let filterFromMe = contract.filters.Transfer(DAO)

// // A filter from anyone to me
// let filterToMe = contract.filters.Transfer(null, DAO)

// // A filter from me AND to me
// let filterFromMeToMe = contract.filters.Transfer(DAO, DAO)

// const divider = ethers.utils.bigNumberify(10).pow(18)

// contract.on(filterFromMe, (fromAddress, toAddress, value, event) => {
//     console.log(
//         'I sent',
//         ethers.utils
//             .bigNumberify(value._hex)
//             .div(divider)
//             .toNumber()
//     )
// })
